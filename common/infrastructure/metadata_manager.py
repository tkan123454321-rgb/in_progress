...
class MetadataManager:
    ...
    def __init__(self, pg_client: PostgresClient, lake_client: LakeHouseClient):
        ...
        """
        self.pg_conn: psycopg.Connection = pg_client.get_db_connection(
    ...
    def _get_ticker_list_raw(
        self, mode: Literal["fundamental", "other_data", "vnindex"] = "fundamental"
    ) -> set[str]:
        """
        ...
        try:
            if mode == "fundamental":
                ...
                tbl = self.catalog.load_table(table)
                tbl = tbl.scan(
                    row_filter=EqualTo(term="status", literal="qualified"),
    ...
    def log_metadata_to_db(
        self, table_name: str, batch_id: str, ticker: str, data_type: str
    ) -> None:
        """
        ...
        try:
            ...
            logger.debug(
                f"Logged metadata for ticker '{ticker}' with batch_id '{batch_id}' and data_type '{data_type}' into Postgres."
            )
        except psycopg.Error as e:
            logger.error(
                f"Database error during metadata logging. Ticker: '{ticker}', "
                f"SQLState: '{e.sqlstate}', Error: {e}",
                exc_info=True,
            )
            raise e
    ...
    def cleanup_kafka_state(self, table_name: str, data_type: str) -> None:
        ...
        if deleted_rows > 0:
            logger.info(
                f"Purged expired Kafka state records (>24h). Table: '{table_name}', Data Type: '{data_type}', Deleted Rows: {deleted_rows}."
            )
        else:
            logger.info(
                f"No expired Kafka state records found. Table: '{table_name}', Data Type: '{data_type}'. State is clean."
            )

    def get_missing_tickers(
        self, table_name: str, tickers_set: set[str], data_type: str
    ):
        """
        ...
        """
        query = (
            f"SELECT ticker FROM ingestion.{table_name} where data_type = '{data_type}'"
        )
        # Polars with ConnectorX is highly optimized for PostgreSQL reads
        df = pl.read_database_uri(
            query=query, uri=self.pg_conn_str, engine="connectorx"
        )
        ...
        if missing_tickers:
            ...
            logger.info(
                f"All active tickers have been successfully ingested in the last 24h. Data Type: '{data_type}'."
            )
    ...
    def sync_watermark(self, config: BaseMetadata) -> None:
        ...
        """
        logger.info(
            f"Starting watermark sync. Reconciling Lakehouse reference list with Postgres watermark table for '{config.data_type}'..."
        )
        if not config.table_watermark_name_postgres:
            raise ValueError(
                f"Missing Postgres Watermark table name in config for data type '{config.data_type}'."
            )
        ...
        if config.data_type == "historical_quotes":
            dividend_changed_tickers = (
                self._get_dividend_changed_tickers() & gold_tickers
            )
        ...
        if not any([new_tickers, disappeared_tickers, reappeared_tickers]):
            logger.info(
                f"Watermark is 100% synchronized. No updates needed for '{config.data_type}'."
            )
            return
        ...
        with self.pg_conn.transaction():
            with self.pg_conn.cursor() as cur:
                ...
                if new_tickers:
                    ...
                    """).format(
                        table=sql.Identifier(config.table_watermark_name_postgres)
                    )
                        ...
                        [
                            {
                                ...
                                "fallback_time": fallback_time_str,
                            }
                            for t in new_tickers
                        ],
                    )
                    logger.info(
                        f"Inserted new tickers. Data Type: '{config.data_type}', Count: {len(new_tickers)}."
                    )
                ...
                if disappeared_tickers:
                    ...
                    cur.executemany(
                        deactivate_query,
                        [
                            {"ticker": t, "data_type": config.data_type}
                            for t in disappeared_tickers
                        ],
                    )
                    logger.warning(
                        f"Deactivated missing tickers. Data Type: '{config.data_type}', Count: {len(disappeared_tickers)}."
                    )
                ...
                if reappeared_tickers:
                    ...
                    cur.executemany(
                        ...
                        [
                            {"ticker": t, "data_type": config.data_type}
                            for t in reappeared_tickers
                        ],
                    )
                    logger.info(
                        f"Reactivated returning tickers. Data Type: '{config.data_type}', Count: {len(reappeared_tickers)}."
                    )
                ...
                if dividend_changed_tickers:
                    ...
                    cur.executemany(
                        ...
                        [
                            ...
                            for t in dividend_changed_tickers
                        ],
                    )
                    logger.info(
                        f"Reset watermarks due to dividend changes. Data Type: '{config.data_type}', Count: {len(dividend_changed_tickers)}."
                    )

        logger.info(
            f"Watermark reconciliation completed successfully. Data Type: '{config.data_type}'."
        )
    ...
    def reconcile_watermark_from_lakehouse(self, config: BaseMetadata) -> None:
        ...
        logger.info(
            f"Starting Lakehouse sync. Scanning actual data to rebuild state for '{config.data_type}'."
        )
        ...
        if not table_exists:
            logger.warning(
                f"⚠️ Bảng bronze.{config.bronze_layer_name} CHƯA TỒN TẠI, bắt đầu hard reset hệ thống."
            )
        ...
        if not results:
            ...
            logger.warning(
                f"No Lakehouse data found. Initiating full state reset. Data Type: '{config.data_type}'."
            )
            ...
            logger.info(
                f"✅ Đã dọn sạch dữ liệu cũ của '{config.data_type}' trong Iceberg."
            )
            ...
            logger.info(
                f"Lakehouse data found. Syncing latest timestamps to Postgres. Data Type: '{config.data_type}'."
            )
            ...
            logger.info(
                f"Watermark state successfully synchronized. Data Type: '{config.data_type}', Updated Count: {len(payload)}."
            )
    ...
    def _reset_postgres_state(self, config: BaseMetadata) -> None:
        ...
        """
        logger.info(
            f"Initiating deep cleanup of Postgres states. Data Type: '{config.data_type}'."
        )
        with self.pg_conn.transaction():
            with self.pg_conn.cursor() as pg_cur:
                ...
                if config.table_watermark_name_postgres:
                    ...
                    pg_cur.execute(
                        cleanup_watermark_sql, {"data_type": config.data_type}
                    )
                    logger.info(
                        f"Cleared watermark states. Table: '{config.table_watermark_name_postgres}'."
                    )
                ...
                if config.table_name_postgres:
                    ...
                    logger.info(
                        f"Cleared Kafka claim-check metadata. Table: '{config.table_name_postgres}'."
                    )

        logger.info(
            f"Postgres state completely reset. Data Type: '{config.data_type}'. Ready for fresh ingestion."
        )
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if exc_type:
            logger.error(
                f"MetadataManager context closed due to an exception: {exc_val}",
                exc_info=True,
            )
        else:
            logger.info(
                "MetadataManager context closed gracefully. All database connections terminated."
            )

{{ config(materialized="table", tags=["gold", "qmj_final"]) }}

{% set audit_cols = get_audit_columns("gold") %}

-- STEP 1: Extract the 3 core pillars of Quality from the Gold layer
with
    gold_profitability as (select * from {{ ref("gold_qmj_z_profitability") }}),

    gold_growth as (select * from {{ ref("gold_qmj_z_growth") }}),

    gold_safety as (select * from {{ ref("gold_qmj_z_safety") }}),

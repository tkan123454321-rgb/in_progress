...
class KafkaClient:
    ...
    def __init__(self, topic_name: str) -> None:
        ...
        self._check_and_create_topic(
            topic_name
        )  # Kiểm tra và tạo topic mặc định ngay khi khởi tạo
            ...
            if topic_name in metadata.topics:
                ...
                for topic, f in future.items():
                    try:
                        ...
                        logger.error(
                            f"Failed to create topic '{topic}': {e}", exc_info=True
                        )
                        return False
        except Exception as e:
            logger.error(
                f"Không thể kết nối đến Kafka tại {self.DEFAULT_BOOTSTRAP}: {e}",
                exc_info=True,
            )
            return False

...
def system_maintenance():
    ...
    @task_group(
        group_id="infrastructure_maintenance",
        group_display_name="Infrastructure Maintenance",
    )
    def infrastructure_maintenance_group():
        jdbc_args = [
            "--jdbc-url",
            "jdbc:postgresql://postgres:5432/platform_db?currentSchema=nessie_gc",
            "--jdbc-user",
            POSTGRES_USER,
            "--jdbc-password",
            POSTGRES_PASSWORD,
        ...
        setup_gc_schema = DockerOperator(
            ...
            command=["create-sql-schema"]
            + jdbc_args
            + ["--jdbc-schema", "DROP_AND_CREATE"],
        ...
        clean_minio_files = DockerOperator(
            ...
            + [
                "-I",
                f"s3.access-key-id={MINIO_ROOT_USER}",
                "-I",
                f"s3.secret-access-key={MINIO_ROOT_PASSWORD}",
                "-I",
        ...
        delete_catalog_tasks = DockerOperator(
            ...
            image="ghcr.io/projectnessie/nessie-server-admin:0.107.4",
            command=["delete-catalog-tasks", "--ref", "main", "--batch", "500"],
            network_mode=DOCKER_NETWORK,
            docker_url="unix://var/run/docker.sock",
            auto_remove="force",
            mount_tmp_dir=False,
        ...
        (
            setup_gc_schema
            >> clean_minio_files
            >> clean_nessie_metadata
            >> delete_catalog_tasks
            >> vacuum_postgres
        )  # type: ignore

    @task(task_display_name="Minio orphan files cleanup")
    def minio_cleanup():
        with LakehouseMaintenance(

...
class KafkaStockConsumer(KafkaClient):
    ...
    def consume_message(self) -> Iterable[dict[str, Any]]:
        ...
        while True:
            try:
                ...
                if not messages:
                    EMPTY_POLL += 1
                    logger.info(f"Empty poll ({EMPTY_POLL}/{self.MAX_EMPTY_POLLS}) for topic '{self.topic_name}'.")
                    if EMPTY_POLL >= self.MAX_EMPTY_POLLS:
                        logger.info("Max empty polls reached. Assuming topic is fully consumed. Halting poll loop.")
                        break
                    continue
                ...
                for msg in messages:
                    ...
                    if msg_error:
                        ...
                        if msg_error.code() == KafkaError._PARTITION_EOF: # type: ignore
                            ...
                            logger.error(f"Message extraction error. Code: {msg_error.code()}, Details: {msg_error.str()}")
                        continue
                    ...
                    try:
                        ...
                        logger.error(f"Failed to decode Kafka message payload. Error: {e}")
                        continue
                    
            except KafkaException as e:
                logger.critical(f"Critical Kafka cluster exception during polling: {e}", exc_info=True)
                raise e
    ...
    def _flush_and_commit(self,buffer: list[dict[str, Any]]) -> None:
        ...
        try:
            ...
            logger.info(f"Kafka offsets committed and populated lakehouse successfully. Partitions: {topic_partitions}")
            ...
            if err.code() == KafkaError._NO_OFFSET: # type: ignore
                ...
                logger.error(f"Critical failure during offset commit: {err}", exc_info=True)
                raise e
        finally:
    ...
    def _process_single_message(request_session: requests.Session, msg: dict[str, Any], transform_callable: Callable[[dict[str, Any], Any], list]) -> list | None:
        ...
        url = msg.get("url")
        if not url:
            logger.warning(f" message skipped. Missing 'url' field. Ticker: '{ticker}', Message: {msg}")
            return None
        try:
            ...
            if not api_data:
                logger.info(f"No data returned from API for ticker '{ticker}'. URL: {url}. Skipping.")
                return None
            else:
                logger.debug(f"Successfully fetched API payload for ticker '{ticker}'. Initiating transformation.")
    ...
    def managed(cls, topic_name: str, update_conf: dict[str, Any] | None = None):
        ...
        """
        logger.info(f"Establishing managed Kafka Consumer context for topic '{topic_name}'...")
        consumer_instance = cls(topic_name=topic_name, update_conf=update_conf)
        
        try:
            yield consumer_instance
        finally:
            logger.info(f"Tearing down Kafka Consumer context for topic '{topic_name}'...")
            try:
                consumer_instance.consumer.close()
                logger.info("Kafka Consumer closed gracefully.")
            except Exception as e:
                logger.error(f"Error encountered while closing Kafka Consumer: {e}", exc_info=True)
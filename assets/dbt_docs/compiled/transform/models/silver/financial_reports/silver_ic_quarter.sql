...
with
    ...
    new_data as (
        ...
        where
            data_type = 'income_statement_quarter'
            and year >= 2018
    ...
    pivoted_data as (
        select
            ...
            quarter,
            MAX(
                case when indicator_id = 1 then CAST(value as DECIMAL(20, 4)) end
            ) as gross_revenue,
            MAX(
                case when indicator_id = 2 then CAST(value as DECIMAL(20, 4)) end
            ) as revenue_deduction,
            MAX(
                case when indicator_id = 3 then CAST(value as DECIMAL(20, 4)) end
            ) as net_revenue,
            MAX(
                case when indicator_id = 4 then CAST(value as DECIMAL(20, 4)) end
            ) as cogs,
            MAX(
                case when indicator_id = 5 then CAST(value as DECIMAL(20, 4)) end
            ) as gross_profit,
            MAX(
                case when indicator_id = 6 then CAST(value as DECIMAL(20, 4)) end
            ) as financial_income,
            MAX(
                case when indicator_id = 7 then CAST(value as DECIMAL(20, 4)) end
            ) as financial_expense,
            MAX(
                case when indicator_id = 701 then CAST(value as DECIMAL(20, 4)) end
            ) as interest_expense,
            MAX(
                case when indicator_id = 8 then CAST(value as DECIMAL(20, 4)) end
            ) as affiliate_profit,
            MAX(
                case when indicator_id = 9 then CAST(value as DECIMAL(20, 4)) end
            ) as selling_expense,
            MAX(
                case when indicator_id = 10 then CAST(value as DECIMAL(20, 4)) end
            ) as admin_expense,
            MAX(
                case when indicator_id = 11 then CAST(value as DECIMAL(20, 4)) end
            ) as operating_profit,
            MAX(
                case when indicator_id = 12 then CAST(value as DECIMAL(20, 4)) end
            ) as other_income,
            MAX(
                case when indicator_id = 13 then CAST(value as DECIMAL(20, 4)) end
            ) as other_expense,
            MAX(
                case when indicator_id = 14 then CAST(value as DECIMAL(20, 4)) end
            ) as other_profit,
            MAX(
                case when indicator_id = 15 then CAST(value as DECIMAL(20, 4)) end
            ) as profit_before_tax,
            MAX(
                case when indicator_id = 16 then CAST(value as DECIMAL(20, 4)) end
            ) as current_tax,
            MAX(
                case when indicator_id = 17 then CAST(value as DECIMAL(20, 4)) end
            ) as deferred_tax,
            MAX(
                case when indicator_id = 18 then CAST(value as DECIMAL(20, 4)) end
            ) as income_tax_expense,
            MAX(
                case when indicator_id = 19 then CAST(value as DECIMAL(20, 4)) end
            ) as net_income,
            MAX(
                case when indicator_id = 20 then CAST(value as DECIMAL(20, 4)) end
            ) as minority_interest,
            MAX(
                case when indicator_id = 21 then CAST(value as DECIMAL(20, 4)) end
            ...
            *,
            ...
            NULLIF(
                CONCAT_WS(
                    ...
                    case
                        when net_revenue is NULL then 'net_revenue is null' else NULL
                    end,

                    case when cogs is NULL then 'cogs is null' else NULL end,

                    case
                        when gross_profit is NULL then 'gross_profit is null' else NULL
                    end,
                    ...
                    case
                        when net_income is NULL then 'net_income is null' else NULL
                    end,
                    ...
                    -- 11 = (5) + (6) - (7) + (8) - (9) - (10): Check Net Operating
                    -- Profit
                    case
                        when
                            ABS(

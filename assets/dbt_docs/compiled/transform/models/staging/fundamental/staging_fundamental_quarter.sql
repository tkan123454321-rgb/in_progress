...
select
    ...
    TRY_CAST(
        json_extract_scalar(value, '$.PreferredStock') as DECIMAL(20, 4)
    ) as preferred_stock,

    TRY_CAST(
        json_extract_scalar(value, '$.MarketCapAtPeriodEnd') as DECIMAL(20, 4)
    ) as market_cap,

    TRY_CAST(
        json_extract_scalar(value, '$.ShareAtPeriodEnd') as DECIMAL(20, 4)

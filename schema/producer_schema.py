...
from typing import Any, List, Tuple, Iterable, Optional, Literal
...
class BaseMetadata(BaseModel, ABC):
    ...
    model_config = ConfigDict(strict=True, str_strip_whitespace=True, extra="ignore")
    ...
    created_at_ts: datetime = Field(
        default_factory=lambda: datetime.now(ZoneInfo("UTC"))
    )
    batch_id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    ticker: str = Field(
        default=""
    )  # Dynamic field (Updated per ticker during message generation)
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if exc_type:
            logger.error(
                f"Pipeline session interrupted for '{self.__class__.__name__}'. Error: {exc_val}",
                exc_info=True,
            )
            return False
        logger.info(
            f"Pipeline session closed successfully. Data Type: '{self.data_type}'."
        )

    def _create_kafka_message(
        self, ticker: str, batch_id: str, **kwargs
    ) -> Tuple[str, bytes]:
        """
    ...
    def _generate_kafka_message(
        self, ticker_list: list[str], metadata_manager: MetadataManager, batch_id: str
    ):
        """
    ...
    def transform_message(self, msg: dict[str, Any], api_data: Any) -> dict:
        ...
        if not isinstance(api_data, list):
            raise TypeError(
                f"Expected API data to be a List, but received: {type(api_data)}"
            )
        processed_time = datetime.now(ZoneInfo("UTC"))
        return {**msg, "data": api_data, "message_processed_time": processed_time}
    ...
    def iceberg_schema(self) -> Schema:
        ...
        if self.bronze_layer_name not in TABLE_REGISTRY:
            raise ValueError(
                f"Schema not found in registry for table: '{self.bronze_layer_name}'"
            )
    ...
    def iceberg_partition_spec(self) -> PartitionSpec:
        ...
        if self.bronze_layer_name not in TABLE_REGISTRY:
            raise ValueError(
                f"Partition Spec not found in registry for table: '{self.bronze_layer_name}'"
            )
    ...
    def _build_arrow_payload_lazy(self, buffer: list[dict[str, Any]]) -> pa.Table:
        ...
        df = lf.select(
            [
                ...
                pl.col("source"),
                pl.col("url"),
                pl.col("created_at_ts").str.to_datetime(
...
class HistoricalQuotes(BaseMetadata):
    ...
    def _generate_kafka_message(
        ...
        if not self.table_watermark_name_postgres:
            raise ValueError(
                f"[{self.data_type}] Watermark table name is required to run this data type."
            )
...
class FinancialReportsQuarter(BaseMetadata):
    ...
    def _generate_kafka_message(
        ...
        for ticker in ticker_list:
            ...
            limit = (self.target_year - start_year) * 4 + (
                self.target_quarter - start_quarter
            )
            limit += 2
            if limit <= 0:
                logger.warning(
...
class FundamentalQuarter(FinancialReportsQuarter):
    ...
    def _generate_kafka_message(
        ...
        for ticker in ticker_list:
            ...
            ticker, message_bytes = self._create_kafka_message(
                ticker=ticker, default_limit=limit, batch_id=batch_id
            )
    ...
    def _build_arrow_payload_lazy(self, buffer: list[dict[str, Any]]) -> Any:
        ...
        df = (
            ...
            .unnest(
                "data"
            )  # STEP 2: Explode the nested structure to access year, quarter, financialValues...
            .select(
                [
                    pl.col("ticker"),
                    pl.col("batch_id"),
...
class FinancialReportsYear(BaseMetadata):
    ...
    def _generate_kafka_message(
        ...
        for ticker in ticker_list:
            ...
            if limit <= 0:
                logger.debug(
                    f"⏭️ [{self.data_type.upper()}] {ticker} is up to date (Start: {start_year}, Target: {self.target_year})."
                )
                continue
...
class Dividend(BaseMetadata):
    ...
    table_watermark_name_postgres: Optional[str] = None
    url_template: str = (
        "https://restv2.{source}.vn/symbols/{ticker}/dividends?count={limit}"
    )
    start_date: date = Field(default_factory=lambda: date(get_fallback_year(), 1, 1))
    end_date: date = Field(default_factory=lambda: date.today() - timedelta(days=1))
    ...
    def url(self) -> str:
        return self.url_template.format(
            source=self.source, ticker=self.ticker, limit=self.default_limit
        )

...
class LakehouseMaintenance:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if exc_type:
            logger.error(
                f"Maintenance context closed abnormally. Exception: {exc_val}",
                exc_info=True,
            )
        else:
            logger.info(
                "Maintenance context closed successfully. All connections terminated."
            )
    ...
    def _prepare_temp_minio_table(self) -> None:
        ...
        logger.debug(
            "Prepared temporary table: 'nessie_gc.temp_minio_object_locations'."
        )
    ...
    def _insert_minio_location_batch(self, batch: Sequence[tuple]) -> None:
        ...
        """
        insert_sql = (
            "INSERT INTO nessie_gc.temp_minio_object_locations (location) VALUES (%s)"
        )
    ...
    def _yield_schema_locations(self) -> Generator[Sequence[tuple], None, None]:
        ...
        for schema_name in self.REQUIRED_SCHEMAS:
            ...
            page_paginator = paginator.paginate(
                Bucket=self.BUCKET_NAME, Delimiter="/", Prefix=prefix_path
            )
            ...
            if "Contents" in page:
                ...
                for obj in page["Contents"]:
                    ...
                    if "Errors" in response:
                        logger.error(
                            f"S3 Deletion Error. Prefix: '{location}', Details: {response['Errors']}"
                        )
    ...
    def minio_maintenance(self) -> None:
        ...
        try:
            ...
            logger.info(
                "Successfully synced MinIO physical object locations to PostgreSQL."
            )
            ...
            if total_deleted == 0:
                logger.info(
                    "MinIO GC completed. Storage is clean, no orphan objects found."
                )
            else:
                logger.info(
                    f"MinIO GC completed successfully. Purged Orphan Count: {total_deleted}."
                )
        except Exception as e:
            logger.error(
                f"MinIO Garbage Collection failed. Transaction rolled back. Error: {e}",
                exc_info=True,
            )
    ...
    def _prepare_maintenance_queue(self):
        ...
        cursor_check = self.pg_conn.execute(
            "SELECT COUNT(*) as total_count, MIN(batch_created_at) as oldest_date FROM iceberg_maintenance.iceberg_optimize;"
        )
        ...
        if row["oldest_date"] is not None:  # type: ignore
            ...
            if row["oldest_date"] < datetime.now(timezone.utc) - timedelta(days=7):  # type: ignore
                logger.info(
                    "Optimization queue expired (older than 7 days). Truncating queue."
                )
                self.pg_conn.execute(
                    "TRUNCATE TABLE iceberg_maintenance.iceberg_optimize;"
                )
                return True
    ...
    def _refresh_queue_from_trino(self) -> None:
        ...
        with self.trino_conn.cursor() as cur:
            ...
            rows = cur.fetchall()
        logger.info(
            f"Retrieved {len(rows)} tables from Trino catalog. Populating optimization queue."
        )
        ...
        logger.info(
            "Successfully populated Postgres optimization queue with Lakehouse tables."
        )
        ...
        with self.trino_conn.cursor() as trino_cur:
            ...
            for task in tasks:
                try:
                    ...
                    trino_cur.execute(
                        f"ALTER TABLE {table_full_name} EXECUTE optimize(file_size_threshold => '128MB')"
                    )
                    trino_cur.fetchall()
                    # Command Trino to clean up and rewrite the Iceberg metadata manifest files.
                    trino_cur.execute(
                        f"ALTER TABLE {table_full_name} EXECUTE optimize_manifests"
                    )
                    trino_cur.fetchall()
                    ...
                    logger.info(
                        f"Optimization completed successfully. Table: '{table_full_name}'."
                    )
                    ...
                    logger.error(
                        f"Optimization failed. Table: '{table_full_name}', Error: {e}"
                    )
    ...
    def run_full_iceberg_maintenance(self) -> None:
        ...
        try:
            ...
            logger.info(
                "Lakehouse Iceberg Maintenance pipeline completed successfully."
            )

        except Exception as e:
            # A final safety net to catch any catastrophic failures in the orchestration flow
            logger.critical(
                f"CRITICAL FAILURE in Iceberg Maintenance pipeline. Error: {e}",
                exc_info=True,
            )

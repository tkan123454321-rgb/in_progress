...
with
    ...
    new_data as (
        ...
        where
            data_type = 'balance_sheet_quarter'
            and year >= 2018
    ...
    pivoted_data as (
        select
            ...
            MAX(
                case when indicator_id = 101 then CAST(value as DECIMAL(20, 4)) end
            ) as current_assets,
            MAX(
                case when indicator_id = 102 then CAST(value as DECIMAL(20, 4)) end
            ) as long_term_assets,
            MAX(
                case when indicator_id = 301 then CAST(value as DECIMAL(20, 4)) end
            ) as total_liabilities,
            MAX(
                case when indicator_id = 302 then CAST(value as DECIMAL(20, 4)) end
            ) as total_equity,
            MAX(
                case when indicator_id = 30101 then CAST(value as DECIMAL(20, 4)) end
            ) as current_liabilities,
            MAX(
                case when indicator_id = 30102 then CAST(value as DECIMAL(20, 4)) end
            ) as long_term_liabilities,
            MAX(
                case when indicator_id = 10101 then CAST(value as DECIMAL(20, 4)) end
            ) as cash_and_equivalents,
            MAX(
                case when indicator_id = 3010105 then CAST(value as DECIMAL(20, 4)) end
            ) as income_taxes_payable,
            MAX(
                case when indicator_id = 3020111 then CAST(value as DECIMAL(20, 4)) end
            ) as retained_earnings,
            MAX(
                case when indicator_id = 3020114 then CAST(value as DECIMAL(20, 4)) end
            ) as minority_interest,
            MAX(
                case when indicator_id = 10202 then CAST(value as DECIMAL(20, 4)) end
            ) as fixed_assets,
            MAX(
                case when indicator_id = 10103 then CAST(value as DECIMAL(20, 4)) end
            ) as short_term_receivables,
            MAX(
                case when indicator_id = 3010101 then CAST(value as DECIMAL(20, 4)) end
            ) as short_term_debt,
            MAX(
                case when indicator_id = 3010102 then CAST(value as DECIMAL(20, 4)) end
            ) as current_portion_lt_debt,
            MAX(
                case when indicator_id = 3010115 then CAST(value as DECIMAL(20, 4)) end
            ) as repo_transactions,
            MAX(
                case when indicator_id = 3010206 then CAST(value as DECIMAL(20, 4)) end
            ) as long_term_debt,
            MAX(
                case when indicator_id = 3010207 then CAST(value as DECIMAL(20, 4)) end
            ...
            *,
            ...
            NULLIF(
                CONCAT_WS(
                    ...
                    case
                        when total_assets is NULL then 'total_assets is null' else NULL
                    end,
                    ...
                    case
                        when total_equity is NULL then 'total_equity is null' else NULL
                    end,

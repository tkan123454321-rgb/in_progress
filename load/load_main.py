...
def _load_main_message(config: BaseMetadata, mode: Literal["run_first", "run_retry"] = "run_first") -> None:
    ...
    if mode == "run_first":
        ...
        logger.info(f"Mode: FIRST RUN. Consuming from '{target_table}'. Forwarding errors to: '{error_topic}'.")
    elif mode == "run_retry":
        target_table = f"{config.data_type}_retry"    # type: ignore 
        error_topic = f"{config.data_type}_dlq"        # type: ignore 
        logger.info(f"Mode: RETRY. Consuming from '{target_table}'. Forwarding unrecoverable errors to DLQ: '{error_topic}'.")
        ...
        try:
            for raw_record in consumer.consume_message():
                ...
                try:# STEP 3: Process the message and fetch actual data from the API
                    ...
                    logger.warning(f"Routing message for ticker '{ticker}' to '{error_topic}' due to error: {e}")
                ...
                if len(buffer) >= BATCH_SIZE:
                    logger.info(f"Batch capacity ({BATCH_SIZE}) reached. Initiating Transform & Load.")
            ...
            logger.warning("Pipeline execution interrupted by user (KeyboardInterrupt). Halting load process.")
        finally: 
            # STEP 6: Process any remaining messages in the buffer before shutting down           
            if buffer:
                logger.info(f"Processing {len(buffer)} remaining records in the buffer before shutdown.")
                try:
                    arrow_table = config._build_arrow_payload_lazy(buffer) # type: ignore
                    loader._put_lakehouse(arrow_table=arrow_table, config=config, mode="append") # type: ignore
                    consumer._flush_and_commit(buffer)
                    logger.info("Successfully processed the remaining buffer during shutdown.")
                except Exception as e:
                    logger.critical(f"Critical failure while processing the remaining buffer: {e}", exc_info=True)
                    raise e
...
def load_main(model_cls: type[BaseMetadata]):
    ...
    try:
        with model_cls() as config: # type: ignore
            ...
            logger.info(f"Batch pipeline completed successfully for '{config.data_type}'.")
    except Exception as e:
        logger.critical(f"Critical pipeline failure. Halting execution. Error: {e}", exc_info=True)
        sys.exit(1)
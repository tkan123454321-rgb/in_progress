...
with
    ...
    pivoted_data as (
        select
            ...
            quarter,
            MAX(
                case when indicator_id = 104 then CAST(value as DECIMAL(20, 4)) end
            ) as cfo,
            MAX(
                case when indicator_id = 212 then CAST(value as DECIMAL(20, 4)) end
            ) as cfi,
            MAX(
                case when indicator_id = 311 then CAST(value as DECIMAL(20, 4)) end
            ) as cff,
            MAX(
                case when indicator_id = 4 then CAST(value as DECIMAL(20, 4)) end
            ) as net_cash_flow,
            MAX(
                case when indicator_id = 10201 then CAST(value as DECIMAL(20, 4)) end
            ) as depreciation,
            MAX(
                case when indicator_id = 201 then CAST(value as DECIMAL(20, 4)) end
            ) as capex,
            MAX(
                case when indicator_id = 101 then CAST(value as DECIMAL(20, 4)) end
            ) as profit_before_tax_cf,
            MAX(
                case when indicator_id = 10301 then CAST(value as DECIMAL(20, 4)) end
            ) as delta_receivables,
            MAX(
                case when indicator_id = 10302 then CAST(value as DECIMAL(20, 4)) end
            ) as delta_inventory,
            MAX(
                case when indicator_id = 10303 then CAST(value as DECIMAL(20, 4)) end
            ) as delta_payables,
            MAX(
                case when indicator_id = 303 then CAST(value as DECIMAL(20, 4)) end
            ) as borrowings_received,
            MAX(
                case when indicator_id = 304 then CAST(value as DECIMAL(20, 4)) end
            ) as debt_repaid,
            MAX(
                case when indicator_id = 308 then CAST(value as DECIMAL(20, 4)) end
            ) as dividends_paid,
            MAX(
            ...
            *,
            ...
            NULLIF(
                CONCAT_WS(
                    ...
                    case when cfo is NULL then 'cfo is null' else NULL end,
                    ...
                    case
                        when ending_cash is NULL then 'ending_cash is null' else NULL
                    end,

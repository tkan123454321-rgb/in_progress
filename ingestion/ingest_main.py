...
def ingest_main[T: BaseMetadata](model_cls: type[T], batch_id: str | None) -> None:
    ...
    metadata_manager = MetadataManager(
        pg_client=PostgresClient(), lake_client=LakeHouseClient()
    )
    try:
        ...
        with (
            metadata_manager as metadata_manager,
            StockTickerProducer.managed(topic_name=config.data_type) as producer,
        ):
            # STEP 3: State Reconciliation
            # 3.1. Fetch the reference list of active tickers from the Lakehouse.
            ticker_list = metadata_manager._get_ticker_list_raw(
                mode=config.ticker_list_mode
            )  # type: ignore
            # 3.2. Clean up expired claim-check states (older than 24h) from previous runs
            metadata_manager.cleanup_kafka_state(
                table_name=config.table_name_postgres, data_type=config.data_type
            )  # type: ignore
            # 3.3. Compare reference list vs today's successful records to find missing ones.
            missing_tickers = metadata_manager.get_missing_tickers(
                table_name=config.table_name_postgres,
            ...
            if not missing_tickers:
                logger.info(
                    f"Pipeline completed early. All tickers for '{config.data_type}' are already processed today."
                )
                return

            logger.info(
                f"Initiating message generation for {len(missing_tickers)} missing tickers. Target Topic: '{config.data_type}'."
            )
            ...
            for ticker, metadata_items in config._generate_kafka_message(
                ...
                is_sent = producer.batch_message_data(
                    messages_list=metadata_items, key=ticker
                )
                ...
                metadata_manager.log_metadata_to_db(
                    ticker=ticker,
                    batch_id=BATCH_ID,
            ...
            logger.info(
                f"Ingestion batch '{BATCH_ID}' completed successfully. Monitoring available on Grafana."
            )
    except KeyboardInterrupt:
        # Graceful shutdown on manual interrupt (Ctrl+C during local dev)
        logger.warning(
            "Pipeline execution interrupted by user (KeyboardInterrupt). Shutting down gracefully."
        )

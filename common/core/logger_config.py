import json
from logging import getLogger, LoggerAdapter, Formatter, StreamHandler
import logging
import sys
...
def setup_logger(
    component: Literal["ingest", "load", "maintenance", "infrastructure"],
    env: str = "dev",
    **kwargs,
) -> LoggerAdapter:
    """
    ...
    if component not in ALLOWED_COMPONENTS:
        raise ValueError(
            f"Component '{component}' is invalid. please choose from {ALLOWED_COMPONENTS}."
        )
    ...
    return logging.LoggerAdapter(
        logger, {"component": component, "env": env, "run_id": kwargs.get("run_id")}
    )
